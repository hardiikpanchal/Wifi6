...
RrMultiUserScheduler::FinalizeTxVector(WifiTxVector& txVector, std::string scheduler_logic, bool ul)
{
  ...
  if(!ul){ // DL code 
    ...
    std::cout<<"Computing DL tx vector\n";
    std::cout << "is DL MU: "<<txVector.IsDlMu()<<"\n";
    ...
    std::cout << "ncentralrutones: "<< nCentral26TonesRus << "\n";
    std::cout << "central26tonerus count: "<< central26TonesRus.size()<<"\n"; 
    std::cout << "DL finalizedtx mcandidatessize: "<<m_candidates.size()<<"\n";
    ...
    if(m_enableBsrp){ // BSRP ON
         
    std::cout<<"Computing UL tx vector\n";
    // compute how many stations can be granted an RU and the RU size
    std::cout << "is UL MU: "<<txVector.IsUlMu()<<"\n";
    std::size_t nRusAssigned = ulCandidates.size();
    
    std::size_t nCentral26TonesRus;
    HeRu::RuType ruType =
        HeRu::GetEqualSizedRusForStations(m_allowedWidth, nRusAssigned, nCentral26TonesRus, scheduler_x);
    std::cout<<"number of RUs assigned for UL : "<<nRusAssigned<<"\n";
    NS_LOG_DEBUG(nRusAssigned << " stations are being assigned a " << ruType << " RU");

    if (!m_useCentral26TonesRus || ulCandidates.size() == nRusAssigned)
    {
        nCentral26TonesRus = 0;
    }
    else
    {
        nCentral26TonesRus = std::min(ulCandidates.size() - nRusAssigned, nCentral26TonesRus);
        NS_LOG_DEBUG(nCentral26TonesRus << " stations are being assigned a 26-tones RU");
    }

    // re-allocate RUs based oFinalizen the actual number of candidate stations
    WifiTxVector::HeMuUserInfoMap heMuUserInfoMap;
    std::swap(heMuUserInfoMap, txVector.GetHeMuUserInfoMap());
    ...
    std::cout << "nRusAssigned: "<< nRusAssigned <<"\n";
    std::cout << "ncentralrutones: "<< nCentral26TonesRus << "\n"; 
    std::cout << "UL finalizedtx mcandidatessize: "<<m_candidates.size()<<"\n";
    std::cout << "finalizedtx ulcandidatessize: "<<ulCandidates.size()<<"\n";

    bool done = false;
    // for (std::size_t i = 0; i < nRusAssigned + nCentral26TonesRus; i++)
    // {
    //      if( ulCandidates.size() || beg==ulCandidates.end()){
    //         done=true;
    //         }
    // //    ns3::HeMuUserInfo mc; 
    // //    if(!done)mc= beg->second.second;
    //     if(!done)std::cout<<"hey\n";
    //     NS_ASSERT(candidateIt != m_candidates.end());
    //     auto mapIt = heMuUserInfoMap.find(candidateIt->first->aid);
    //   //  auto y  = mapIt->first;
    //     NS_ASSERT(mapIt != heMuUserInfoMap.end());
    // //   if(!done) std::cout<<"Map AID : "<< y<<"ulCandidates AID: "<< beg->second.first<<"\n";
    //     txVector.SetHeMuUserInfo(mapIt->first,
    //                              {(i < nRusAssigned ? *ruSetIt++ : *central26TonesRusIt++),
    //                               mapIt->second.mcs,
    //                               mapIt->second.nss});
   ...
   for (std::size_t i = 0; i < nRusAssigned + nCentral26TonesRus; i++)
    {
        if( ulCandidates.empty() || beg==ulCandidates.end()){
            done=true;
            }
        if(done)break;
        uint16_t mc; 
        // NS_ASSERT(heMuUserInfoMap.find(beg->second) != heMuUserInfoMap.end());
        
        mc= beg->second.first;
        auto temp = beg->second.second;
        std::cout << "station with AID:"<<mc<< " assigned RU for UL which has queue size: "<<beg->first<<"\n";
            
        txVector.SetHeMuUserInfo(mc,
                                 {(i < nRusAssigned ? *ruSetIt++ : *central26TonesRusIt++),
                                  temp.mcs,
                                  temp.nss});
